...
export const CanManageProperties = () =>
  applyDecorators(
    ...
    ApiForbiddenResponse({ description: 'Forbidden — requires ADMIN, SUPER_ADMIN or CLIENTE role' }),
  );

export const CanManageTenants = () =>
  applyDecorators(
    Roles(Role.ADMIN, Role.SUPER_ADMIN, Role.CLIENTE, Role.VENDEDOR),
    ApiForbiddenResponse({ description: 'Forbidden — requires ADMIN, SUPER_ADMIN, CLIENTE or VENDEDOR role' }),
  );
...
export const CanManagePayments = () =>
  applyDecorators(
    ...
    ApiForbiddenResponse({ description: 'Forbidden — requires ADMIN, SUPER_ADMIN, CLIENTE or FINANZAS role' }),
  );

export const CanManageMaintenance = () =>
  applyDecorators(
    Roles(Role.ADMIN, Role.SUPER_ADMIN, Role.CLIENTE, Role.MANTENIMIENTO),
    ApiForbiddenResponse({ description: 'Forbidden — requires ADMIN, SUPER_ADMIN, CLIENTE or MANTENIMIENTO role' }),
  );
